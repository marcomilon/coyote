...
- **Local development has no mock backend.** `npm run dev` serves only the static frontend; the API, Lambdas, and generated sites are the deployed `dev` stack. The `dev` stack (never `prod`) additionally allows `http://localhost:5173` in CORS and in the sites' `frame-ancestors`, and has a raised rate limit.
