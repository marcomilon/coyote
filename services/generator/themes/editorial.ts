...
.label{font-family:var(--font-body);font-weight:400;font-size:.78rem;line-height:1.55;letter-spacing:.16em;text-transform:uppercase;margin:0 0 2rem;color:var(--accent)}
...
.about .label{grid-column:4/12;margin:0}
.about p{grid-column:4/12;margin:0;font-family:var(--font-display);font-size:clamp(1.35rem,2.6vw,2rem);line-height:1.35}
.about p::first-letter{font-size:3.1em;float:left;line-height:.85;padding:.06em .1em 0 0;color:var(--accent);font-weight:var(--font-display-weight)}
...
@media (max-width:48rem){.hero h1,.hero .side,.signature,.about .label,.about p{grid-column:1/-1}.hero .side{border:0;padding:0}.index{columns:1}.visit{grid-template-columns:1fr}}
`,
...
<section><h2 class="label">${t.services}</h2>
...
<section class="about"><h2 class="label">${t.about}</h2><p>${content.about}</p></section>
${photos.length > 1 ? html`<section><div class="photos">${photos.slice(1)}</div></section>` : ''}
${hasVisit(content) ? html`<section><h2 class="label">${t.visit}</h2><div class="visit">
